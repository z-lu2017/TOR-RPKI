...
def user_specified_client2(consensus_date, numClients, csvfile, numIPdict, rovset, year, month, date, countries_last, cweights_last, generateClients = False, method = 0):
    '''
    ...
    print("sanity check len countries = ", len(countries))
    print("sanity check len of cweights = ", len(cweights))
    ...
    roa_client_percentage = sum(croas)/len(croas)
    rov_client_percentage = sum(crovs)/len(crovs)
    print("client rov coverage == ", rov_client_percentage)
    print("client roa coverage == ", roa_client_percentage)
    print("===========================================================")
    ...
    return roa_client_percentage, rov_client_percentage, countries, cweights
...
def run_sim():
    ...
    for y in years2:
        for m in months2:
            ...
            for d in days:
                ...
                # find routeview data for that day
                os.chdir(root)
                os.chdir("../routeviews")
                ...
                for h in hours:
                    ...
                    #     m1.append(matched_perc[i])
                    #     m2.append(matched_perc2[i])
                    roa_client_percentage, rov_client_percentage, countries, cweights = user_specified_client2(consensus_date_with_hour, 1000000, roa_file, numIPdict, ROVset, y, m, d, countries_last, cweights_last, True, 2)
                    date_strings.append(str(y)+ "-" + str(m) + "-" + str(d))
                    case_strings.append("RoVISTA")
                    m1.append(roa_client_percentage)
                    m2.append(rov_client_percentage)

                    # update global variable last list
                    countries_last = copy.deepcopy(countries)
                    cweights_last = copy.deepcopy(cweights)
                    ...
                    print("finished year = " + y + " month = " + m + " date = " + d +  " hour = " + h)    
    ...
    d = {'date': date_strings, 'case': case_strings, 'roa_perc': m1, "rov_perc": m2}
    df = pd.DataFrame(data = d)
    df.to_csv("./roa-rov-client-Feb2023.csv", index=False)

...
# main function for generating users and simulate choosing guards
def user_specified_client2(consensus_date, numClients, csvfile, numIPdict, rovset, saveClients = False, method = 0):
    ...
    # lists used to record statistics for selected relays from yaml
    roas = []
    rovs = []
    croas = []
    crovs = []
    bws = [] # bandwidths for all guard relays
    ids = [] # ids for all guard relays

    # read in statistics for all relays
    ...
    # read in yaml - preprocess yaml before simulation
    fname = 'shadow.config.yaml'
    stream = open(fname, 'r')
    data = yaml.load(stream, Loader=yaml.FullLoader)

    # count roa and rov ips in the yaml config
    ...
    # case 1 - add asn from MANRS to the list of ROV covered - high critera
        ...
        with open('/home/zzz/Downloads/manrs-rov-high.txt') as txtfile:
            for line in txtfile:
    ...
    # case 3 - add asn from Shulman group
        ...
        with open('/home/zzz/Downloads/protected.txt') as txtfile:
            for line in txtfile:
                rovset.append(str(line))
        rovset = list(dict.fromkeys(rovset))
    # case 4 - add asn from MANRS to the list of ROV covered - low critera
        ...
        with open('/home/zzz/Downloads/manrs-rov-low.txt') as txtfile:
            for line in txtfile:
                rovset.append(str(line))
        rovset = list(dict.fromkeys(rovset))
    ...
    if saveClients:
        ...
        while count < numClients:
            ...
            # record if generated client has ROA adn ROV coverage
    ...
    # generate dummy arrays used in optimization based on generated clients info
    croas_both = []
    crovs_both = []
    ...
    print("both = clientdist 0 = ", client_dist[0])
    print("clientdist 1 = ", client_dist[1])
    print(":clientdist 2= ", client_dist[2])
    print("clientdist 3 = ", client_dist[3])

    # feed all input to optimization 
    optimized_weights = lp_optimization(bws, roas, rovs, croas_both, crovs_both, croas_roa, crovs_roa, croas_rov, crovs_rov, croas_neither, crovs_neither, client_dist)
    ...
    d = {'identity': ids, 'roa': roas, 'rov': rovs, 'weights': optimized_weights1}
    ...
    d = {'identity': ids, 'roa': roas, 'rov': rovs, 'weights': optimized_weights2}
    df = pd.DataFrame(data = d)
    ...
    d = {'identity': ids, 'roa': roas, 'rov': rovs, 'weights': optimized_weights3}
    df = pd.DataFrame(data = d)
    ...
    d = {'identity': ids, 'roa': roas, 'rov': rovs, 'weights': optimized_weights4}
    df = pd.DataFrame(data = d)
...
# start function - starts simulation from date specified
def run_sim():
    ...
    # open a file where a list of ASN with ROV is kept
    file =  open('./ASNwROV.pickle', 'rb')  
    ROVset = list(pickle.load(file))
    file.close()

    # start iteration, days hard-coded here for simplicity
    for y in years:
        for m in months:
            ...
            # iterate in hours, which is the basic unit for consensuses
            for h in hours:
                ...
                print("finished year = " + y + " month = " + m + " hour = " + h + " using Rovista")    

...
def calc_roa_rov_client(countries, cweights, numIPdict, csvfile, rovset):
        ...
        for asn in ASarray:
            ...
            for i in range(len(prefixes)):
                ...
                if roa == True and rov == True:
                    both_prefixes_out.append(prefixes[i])
                if roa == True and rov == False:
                    roa_prefixes_out.append(prefixes[i])
                if roa == False and rov == True:
                    rov_prefixes_out.append(prefixes[i])
                if roa == False and rov == False:
                    neither_prefixes_out.append(prefixes[i])
    ...
    return both_prefixes_out, roa_prefixes_out, rov_prefixes_out, neither_prefixes_out
...
def user_specified_client2(consensus_date, numClients, csvfile, numIPdict, rovset, method = 0):
    ...
    both_prefixes, roa_prefixes, rov_prefixes, neither_prefixes = calc_roa_rov_client(countries, cweights, numIPdict, csvfile, rovset)
        
    return both_prefixes, roa_prefixes, rov_prefixes, neither_prefixes
...
def run_sim(start_date_global):
    ...
    for h in hours:
        ...
        both_prefixes, roa_prefixes, rov_prefixes, neither_prefixes = user_specified_client2(consensus_date_with_hour, 1000000, roa_file, numIPdict, ROVset, 2)
        
        file = open('both-prefixes.txt', 'w')
        for item in both_prefixes:
            file.write(str(item))
            file.write("\n")
        file.close()

        file = open('roa-prefixes.txt', 'w')
        for item in roa_prefixes:
            file.write(str(item))
            file.write("\n")
        file.close()

        file = open('rov-prefixes.txt', 'w')
        for item in rov_prefixes:
            file.write(str(item))
            file.write("\n")
        file.close()

        file = open('neither-prefixes.txt', 'w')
        for item in neither_prefixes:
            file.write(str(item))
            file.write("\n")
        file.close()

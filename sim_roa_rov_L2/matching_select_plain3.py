...
def matching_post_new(consensus_date, clients_both, clients_roa, clients_rov, clients_neither, relayWeights, relay_roas, relay_rovs, client_dist, gs, WGG, WGD, rovset):
    ...
    croas_both = []
    crovs_both = []
    for i in range(len(clients_both)):
        croas_both.append(1)
        crovs_both.append(1)

    croas_roa = []
    crovs_roa = []
    for i in range(len(clients_roa)):
        croas_roa.append(1)
        crovs_roa.append(0)

    croas_rov = []
    crovs_rov = []
    for i in range(len(clients_rov)):
        croas_rov.append(0)
        crovs_rov.append(1)

    croas_neither = []
    crovs_neither = []
    for i in range(len(clients_neither)):
...
def user_specified_client2(consensus_date, rovset, method = 0):
    '''
    ...
    file = open("../clients_daily/1000000TorClients-" + next_date + ".pickle", 'rb')
...
def run_sim(start_date_global, end_date_global):
    hours = ['00']
    ...
    while consensus_date_formal != end_date_global:
        ...
        for h in hours:
            ...
            matched2 = user_specified_client2(consensus_date_with_hour, ROVset, 2)
